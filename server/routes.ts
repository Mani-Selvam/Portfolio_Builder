...
const upload = multer({
    ...
    fileFilter: (req, file, cb) => {
        if (file.fieldname === "resume") {
            ...
            file.fieldname.startsWith("projectScreenshot")
        ) {
    ...
    app.post(
        ...
        async (req, res) => {
            try {
                ...
                const { projects: projectsData, ...submissionOnly } =
                    validatedData;
                ...
                if (projectsData && projectsData.length > 0) {
                    const projectsWithScreenshots = projectsData.map(
                        (project, index) => {
                            let screenshotUrl = null;
                            if (
                                files.projectScreenshots &&
                                files.projectScreenshots[index]
                            ) {
