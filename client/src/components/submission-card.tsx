          ...
          <div className="flex items-start space-x-4 mb-4 lg:mb-0">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center space-x-3 mb-2">
                ...
                <Badge className={`px-3 py-1 text-xs font-medium rounded-full flex items-center gap-1 ${getStatusColor(submission.status)}`}>
                  {getStatusIcon(submission.status)}
                  {submission.status.charAt(0).toUpperCase() + submission.status.slice(1).replace('-', ' ')}
                </Badge>
              </div>
              <p className="text-sm text-gray-600 mb-1">{submission.professionalTitle}</p>
              <p className="text-sm text-gray-500">{submission.email}</p>
              <p className="text-sm text-gray-700 mt-2 line-clamp-2">{submission.bio}</p>
            </div>
          </div>
                ...
                <div className="absolute right-0 top-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg z-10 min-w-[150px]">
                  <div className="p-2">
                    <Select
                      ...
                      onValueChange={(value) => {
                        onStatusUpdate(value, value === 'completed');
          ...
          <div className="flex flex-wrap gap-4 text-sm text-gray-600">
            ...
            {submission.projects.length > 0 && (
              <div className="flex items-center">
                ...
                <span>{submission.projects.map(p => p.technologies).filter(Boolean).join(', ').substring(0, 50)}...</span>
              </div>
            )}
          </div>
        </div>
